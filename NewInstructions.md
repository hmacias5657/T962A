...
**Base Project**: T962A oven conversion, ESP32 WROOM dual-core, 128x64 OLED, 2x K-type TC via ADS1015
...
│   ├── DisplayRenderer.h / .cpp    # SSD1306 via U8g2 ESP-IDF, 11-state menu machine, live plot, 8-min axes
...
├── components/
│   └── u8g2/                       # U8g2 library as ESP-IDF component (managed via IDF component registry or manual)
...
|---|---|
| `Arduino.h` | `esp_log.h`, `freertos/FreeRTOS.h`, `driver/gpio.h`, `driver/ledc.h`, `driver/spi_master.h`, `driver/i2c_master.h`, `nvs_flash.h` |
...
| `U8G2_SSD1306_128X64_NONAME_1_4W_HW_SPI(rotation, cs, dc, rst)` | `U8G2_SSD1306_128X64_NONAME_1_4W_HW_SPI(rotation)` + `u8x8_SetPin(u8g2_GetU8x8(&display), pins...)` |
...
U8G2 must be added as an ESP-IDF component. Two options:

### Option A: IDF Component Registry
```bash
idf.py add-dependency "olikraus/u8g2^2.35.4"
```

### Option B: Manual component directory
Create `components/u8g2/` with the U8g2 source + a `CMakeLists.txt`:

```cmake
idf_component_register(SRCS "u8g2_hw_i2c.c" "..."
                       INCLUDE_DIRS "."
                       REQUIRES driver)
```
...
**Arduino version** (`DisplayRenderer.cpp`):
```cpp
...
**ESP-IDF version**:
```cpp
U8G2_SSD1306_128X64_NONAME_1_4W_HW_SPI _display(U8G2_R0);
// In begin():
u8x8_SetPin(u8g2_GetU8x8(&_display), U8X8_PIN_CS, PIN_DISPLAY_CS);
u8x8_SetPin(u8g2_GetU8x8(&_display), U8X8_PIN_DC, PIN_DISPLAY_DC);
u8x8_SetPin(u8g2_GetU8x8(&_display), U8X8_PIN_RESET, PIN_DISPLAY_RST);
```

SPI bus must be initialized in `app_main()` before U8G2:
...
extern "C" void app_main() {
    // 1. Initialize NVS
    // 2. Initialize SPI bus (display)
    // 3. Initialize I2C bus (ADS1015)
    // 4. Initialize GPIO (SSR, ZC, buttons, buzzer)
    // 5. Initialize LEDC (system fan)
    // 6. Load NVS data (recipes, settings, calibration)
    // 7. Create objects
    // 8. Create queues / mutexes
    // 9. Create Core 0 + Core 1 tasks
    // 10. Delete self task (vTaskDelete(NULL))
}
```
...
- [ ] **SPI (Display)**: Add `spi_bus_initialize(SPI2_HOST, ...)` before U8G2 init. Use zero-argument U8G2 constructor + `u8x8_SetPin`.
...
- 8-minute live temperature plot (280°C Y-axis, 128x64 OLED SSD1306 SPI)
...
- °C/°F unit toggle
- Calibration Run (auto-tune recipe times with 1.2x margin)
